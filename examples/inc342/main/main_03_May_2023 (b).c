...
void byte_received(void *evt)
{
	uart_event_t *ue = (uart_event_t *)evt;
	
	//Uart1_Printf("byte: 0x%.2X %d\r\n", ue->byte, ue->byte);
	/**
	 * Q1: '0' --> LED0 OFF
	 *     '1' --> LED0 ON
	 *     '2' --> LED0 TOGGLE
	*/
	...
	if( (ue->byte == '\r') || (ue->byte == '\n') ) {
		return;
	}

	if( ((ue->byte >= '0') && (ue->byte <= '2')) || (ue->byte == 'f')) {
		if(ue->byte == '0') {
			Led_Clr(LED_ID_0); 	Uart1_Printf("LED0 OFF\r\n");	
		}
		else if(ue->byte == '1') {
			Led_Set(LED_ID_0);	Uart1_Printf("LED0 ON\r\n");	
		}
		else if(ue->byte == '2') {
			Led_Inv(LED_ID_0);	Uart1_Printf("LED0 TOGGLE\r\n");	
		}
		else if(ue->byte == 'f') {
			Led_Flash(LED_ID_0, 500);	Uart1_Printf("LED0 Flash\r\n");	
		}
	}
	else {
		Uart1_Printf("Error!\r\n");	
...
void line_received(void *evt) {
	uart_event_t *ue = (uart_event_t *)evt;

	// Uart1_Printf("line: %d %s", ue->length, ue->line);
	...
	// }
	// else {
	// 	Uart1_Printf("Error: wrong command!\r\n");	
	// }
	...
	*/
	...
	Uart1_Printf("Error: Command is not allowed!\r\n");
}
...
int main(void)
{
	...
	// For Q2-3
	Uart1_SetRxLineCallback(line_received, 16);
